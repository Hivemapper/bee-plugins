...
def calibration():
    with paramiko.SSHClient() as ssh:
        ...
        return json.loads(
            str(run_command_over_ssh(ssh, "cat /data/cache/calibration.json")).strip()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-C", "--calibration", help="Device calibration profile", action="store_true"
    )
    ...
    parser.add_argument(
        "-Ws", "--wifi_scan", help="Show visible WiFi networks", action="store_true"
    )
