...
def get_plugin_id(base_url: str, plugin_name: str, plugin_secret: str) -> str:
    ...
    if not plugin_id:
        raise ValueError(
            f"Could not resolve _id for plugin '{plugin_name}' — response: {json.dumps(data)}"
        )
    return plugin_id


def upload_encrypted_secrets(
    base_url: str, plugin_name: str, plugin_secret: str, encrypted_blob: str
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Encrypt and upload plugin secrets to Hivemapper backend."
    )
    parser.add_argument(
        "--plugin-name", required=True, help="Plugin name as registered in Hivemapper"
    )
    parser.add_argument(
        "--plugin-secret", required=True, help='Plugin API key (the "secret" field in MongoDB)'
    )
    parser.add_argument(
        "--env-file", required=True, help="Path to .env file containing key=value pairs"
    )
    parser.add_argument(
        "--base-url",
        default=DEFAULT_BASE_URL,
        help=f"Hivemapper backend URL (default: {DEFAULT_BASE_URL})",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Encrypt and print blob without uploading"
    )
    ...
    result = upload_encrypted_secrets(
        args.base_url, args.plugin_name, args.plugin_secret, encrypted_blob
    )
