...
def test_returns_file_paths():
    ...
    with patch('beeutil.recordings.requests.get', return_value=mock_resp) as mock_get:
        ...
        assert len(result) == 2
        assert result[0] == {'filepath': '/data/video/100.mp4', 'filename': '100.mp4', 'timestamp_ms': 100}
        assert result[1] == {'filepath': '/data/video/110.mp4', 'filename': '110.mp4', 'timestamp_ms': 110}
